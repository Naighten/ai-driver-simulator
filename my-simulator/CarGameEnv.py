...
class CarGameEnv(gym.Env):
    ...
    def __init__(
        ...
        self.default_reward_config = {
            # Базовые параметры
            "checkpoint": 100.0,  # Бонус за чекпоинт
            "lap_complete": 1000.0,  # Бонус за полный круг
            "speed_coef": 3.0,  # Множитель награды за скорость
            ...
            "min_speed_penalty": 2.0,  # Штраф за низкую скорость
            "angle_penalty_coef": 0.05,  # Штраф за резкие повороты
            "proximity_coef": 0.7,  # Множитель награды за приближение к чекпоинту
            "max_proximity_bonus": 3.0,  # Максимальный бонус за шаг
            # Пороговые значения
        ...
        self.track_max_reward = track_cfg.get("max_reward", None)
    ...
    def _get_obs(self, x, y):
        ...
        # Явно проверяем количество сенсоров
        assert len(self.game.sensor_data) == 8, "Должно быть 8 значений сенсоров"
        ...
        obs = np.array(
            [
                ...
                *self.game.sensor_data,  # 8 значений
            ],
            dtype=np.float32,
        )

        # Проверка окончательного размера
        if len(obs) != 12:
            raise ValueError(f"Некорректный размер наблюдения: {len(obs)} вместо 12")
    ...
    def _calculate_checkpoint_reward(self, x, y):
        ...
        if self.passed_all_checkpoints:
            return reward
    ...
    def _check_lap_completion(self):
        ...
        if self.game.laps >= 1 and self.next_wp_idx >= 1:
            if self.passed_all_checkpoints:
