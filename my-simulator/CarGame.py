...
class CarGame:
    ...
    def __init__(self, track_name="newtrack2", render_mode="human", seed=None):
        ...
        match self.render_mode:
            case "human":
                ...
                self.track = pygame.image.load(
                    "tracks/" + self.track_data["track_image"]
                ).convert()
            case _:
                self.screen = None
                self.font = None
        ...
        self.startline_coords = {
            "x_min": self.track_data["startline_coords"]["x_min"],
            "x_max": self.track_data["startline_coords"]["x_max"],
            "y_min": self.track_data["startline_coords"]["y_min"],
            "y_max": self.track_data["startline_coords"]["y_max"],
        }
    ...
    def cast_ray(self, angle):
        ...
        for _ in range(int(steps)):
            ...
            if not (0 <= ix < self.WIDTH and 0 <= iy < self.HEIGHT):
                edge_dist = self._distance_to_edge(
    ...
    def update_sensors(self):
        ...
        for angle in self.sensor_angles:
            val = self.cast_ray(angle)
            self.sensor_data.append(float(val))

        if len(self.sensor_data) != 8:
            raise RuntimeError(f"Количество сенсоров: {len(self.sensor_data)} вместо 8")
    ...
    def update_physics(self, steering: int, throttle: int):
        ...
        if self.on_track(self.car_x, self.car_y):
            ...
            if (
                self.previous_f_result < f_result
                and self.startline_coords["x_min"]
                <= self.car_x
                <= self.startline_coords["x_max"]
            ):
                self.laps += 1
    ...
    def on_track(self, x, y):
        ...
        if (
            self.startline_coords["x_min"] <= x <= self.startline_coords["x_max"]
            and self.startline_coords["y_min"] <= y <= self.startline_coords["y_max"]
        ):
            return True
    ...
    def render(self):
        ...
        for i, (distance, color) in enumerate(
            zip(self.sensor_data, self.sensor_colors)
        ):
            angle = self.car_angle + self.sensor_angles[i]
            rad = math.radians(angle)
            length = (
                distance
                * self.max_sensor_length
